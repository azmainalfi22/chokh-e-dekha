...
<div class="relative">
  ...
  <div class="max-w-4xl mx-auto p-4 md:p-8 relative">
    ...
    <form action="{{ route('report.store') }}" method="POST" enctype="multipart/form-data"
          class="bg-white/80 backdrop-blur rounded-2xl shadow p-6 space-y-5 ring-1 ring-amber-100">
      @csrf
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        {{-- Category (dynamic from controller + fallback) --}}
        <div>
          <label for="category" class="block text-sm font-medium text-gray-700 mb-1">Category <span class="text-rose-600">*</span></label>
          @php
            // Fallback if controller didn’t pass categories for any reason
            $cats = (isset($categories) && is_array($categories) && count($categories))
                      ? $categories
                      : [
                          'Road Damage','Broken Road','Street Light','Electricity',
                          'Water Supply','Drainage','Waste Management','Garbage',
                          'Sewage','Public Safety','Traffic','Parks',
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        {{-- City Corporation --}}
        <div>
          <label for="city_corporation" class="block text-sm font-medium text-gray-700 mb-1">
      ...
      <div>
        ...
        <input id="photo" type="file" name="photo" accept="image/*"
               class="block w-full text-sm text-gray-700 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:bg-amber-600 file:text-white hover:file:bg-amber-700">
      ...
      <div class="flex items-center justify-between">
        ...
        <button type="submit"
                class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-amber-600 text-white hover:bg-amber-700 shadow">
          <span>Submit</span>
